...
#define I2C_ERROR_INTS                                                         \
  (I2C_TIME_OUT_INT_RAW | I2C_ARBITRATION_LOST_INT_RAW | \
...
bool mgos_i2c_set_freq(struct mgos_i2c *c, int freq) {
  ...
  dev->timeout.time_out_value = 16;  // I2C_FIFO_LEN * 9 * 2  + 5 ;
...
static bool esp32_i2c_exec(struct mgos_i2c *c, int num_cmds, int exp_tx,
  ...
  uint32_t old_fifo_cnt =
      dev->sr.tx_fifo_cnt + dev->sr.rx_fifo_cnt;
  ...
  if (ok) {
    if ((exp_tx >= 0 && dev->sr.tx_fifo_cnt != exp_tx) ||
        (exp_rx >= 0 && dev->sr.rx_fifo_cnt != exp_rx)) {
      ok = false;
    }
  }
  if (c->debug) {
    LOG(LL_DEBUG,
        ("  %d loops, ok? %d, ints 0x%08x -> 0x%08x, "
         ...
         loops, ok, ints_before, ints, dev->sr.tx_fifo_cnt, exp_tx,
         dev->sr.rx_fifo_cnt, exp_rx, dev->sr.val, dev->ctr.val,
         c->num_bus_resets));
    for (int ci = 0; ci < num_cmds; ci++) {
      LOG(LL_DEBUG,
          ("    %08x %d", dev->command[ci].val, dev->command[ci].command0 >> 11));
    }
  }
  ...
  if (((ints & I2C_TIME_OUT_INT_RAW) || (loops >= max_loops)) &&
      !(ints & I2C_BYTE_TRANS_DONE_INT_RAW)) {
...
bool mgos_i2c_write(struct mgos_i2c *c, uint16_t addr, const void *data,
  ...
  if (len == 0 && (ci > 0 || (stop && !c->stopped))) {
    ...
    while (di < len && ok) {
      int tx_len = len - di;
      int tx_fifo_len = dev->sr.tx_fifo_cnt;
